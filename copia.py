...
if __name__ == "__main__":
    ...
    # Initialize m2 to avoid NameError
    ...
    if source_port == destination_port:
        ...
        # Añadir los puntos de viento, temperatura y humedad a lo largo de la ruta
        for coord in coord_path:
            lat, lon = coord  # Extraer latitud y longitud de las coordenadas
            weather_desc, temp, wind_speed, wind_direction = get_weather_data(lat, lon)
        
            if weather_desc:  # Solo agregar si los datos meteorológicos están disponibles
                # Seleccionar ícono basado en el clima
                if "clear" in weather_desc:  # Clima despejado
                    icon = "sun"
                    color = "orange"
                elif "cloud" in weather_desc:  # Nubes
                    icon = "cloud"
                    color = "gray"
                elif "rain" in weather_desc:  # Lluvias
                    icon = "cloud-rain"
                    color = "blue"
                elif "thunderstorm" in weather_desc:  # Tormentas
                    icon = "bolt"
                    color = "darkblue"
                elif "snow" in weather_desc:  # Nieve
                    icon = "snowflake"
                    color = "lightblue"
                elif temp and temp > 35:  # Temperaturas altas (representar calor extremo)
                    icon = "fire"
                    color = "red"
                else:
                    icon = "info-sign"  # Ícono genérico si no hay información clara
                    color = "green"

                # Crear información emergente (popup) con los detalles del clima
                popup_info = f"<b>Clima:</b> {weather_desc}<br>" \
                            f"<b>Temperatura:</b> {temp}°C<br>" \
                            f"<b>Viento:</b> {wind_speed} m/s<br>" \
                            f"<b>Dirección del Viento:</b> {wind_direction}°"
            
                # Agregar marcador al mapa
                folium.Marker(
                    location=[lat, lon],
                    icon=folium.Icon(color=color, icon=icon, prefix='fa'),  # Usamos íconos FontAwesome
                    popup=popup_info,
                    tooltip="Información del Clima"
                ).add_to(m2)

    # Mostrar el mapa con la ruta trazada y la información meteorológica
    folium_static(m2)